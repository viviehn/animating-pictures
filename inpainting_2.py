    ...
    for y in range(radius+1, h - radius):
        for x in range(radius+1, w - radius):
            ...
            if not np.isin(1, patch[:,:,0]):
                src_patch, size = get_patch(x, y, radius, cielab_im*cur_mask[:,:,np.newaxis])
...
def copy_patch(t_x, t_y, s_x, s_y, cur_image, cur_mask, cur_conf, radius):
    update_image = cur_image
    update_mask = cur_mask
    update_conf = cur_conf
    h,w = cur_image.shape[:2]
    for y in range(-radius, radius+1):
        for x in range(-radius, radius+1):
            if cur_mask[t_y-y, t_x-x] == 1:
...
def inpaint(mask, bg):
    ...
    while not done:
        ...
        print(len(fill_front[0]))
        priority_map = calc_priority(conf_map, cur_image, cur_mask, fill_front, radius)
        target_y, target_x = np.unravel_index(priority_map.argmax(), priority_map.shape)
